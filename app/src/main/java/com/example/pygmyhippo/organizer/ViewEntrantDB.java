                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        if (events.size() == 0) {
                            ...
                            listener.OnCompleteDB(events, 0, DBOnCompleteFlags.NO_DOCUMENTS.value);
                        } else if (events.size() == 1) {
                            Log.d("DB", String.format("Found event (%s) with event ID (%s)", events.get(0).getEventID(), eventID));
                            listener.OnCompleteDB(events, 0, DBOnCompleteFlags.SINGLE_DOCUMENT.value);
                        } else {
                            Log.d("DB", String.format("Found %d events  with event ID (%s)", events.size(), eventID));
                            listener.OnCompleteDB(events, 0, DBOnCompleteFlags.MULTIPLE_DOCUMENTS.value);
                        }
                    } else {
                ...
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        ...
                        if (accounts.size() == 0) {
                            ...
                            listener.OnCompleteDB(accounts, 1, DBOnCompleteFlags.NO_DOCUMENTS.value);
                        } else if (accounts.size() == 1) {
                            Log.d("DB", String.format("Found account (%s) with account ID (%s)", accounts.get(0).getAccountID(), accountID));
                            listener.OnCompleteDB(accounts, 1, DBOnCompleteFlags.SINGLE_DOCUMENT.value);
                        } else {
                            Log.d("DB", String.format("Found %d accounts  with account ID (%s)", accounts.size(), accountID));
                            listener.OnCompleteDB(accounts, 1, DBOnCompleteFlags.MULTIPLE_DOCUMENTS.value);
                        }
                    } else {
